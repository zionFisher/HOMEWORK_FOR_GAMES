...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    //     for (int y = ymin; y <= ymax; y++)
    //     {
    //         float X[4] = {x + 0.0f, x + 0.5f, x + 0.0f, x + 0.5f};
    //         float Y[4] = {y + 0.0f, y + 0.0f, y + 0.5f, y + 0.5f};
    //         int INDEX[4] = {get_MSAA_index(X[0], Y[0]), 
    //                         get_MSAA_index(X[1], Y[1]),
    //                         get_MSAA_index(X[2], Y[2]),
    //                         get_MSAA_index(X[3], Y[3])};
    //         Vector3f COLOR[4];
    //         bool insideFlag = false, depthFlag = false;
    ...
    //             {
    //                 insideFlag = true;
    //                 auto [alpha, beta, gamma] = computeBarycentric2D(X[count], Y[count], t.v);
    //                 float w_reciprocal = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
    //                 float z_interpolated = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
    //                 z_interpolated *= w_reciprocal;
    //                 auto depth = INDEX[count];
    //                 if (z_interpolated < MSAA_depth_buf[depth])
    //                 {
    //                     depthFlag = true;
    //                     MSAA_depth_buf[depth] = z_interpolated;
    //                     auto interpolated_color = interpolate(alpha, beta, gamma, t.color[0], t.color[1], t.color[2], 1);
    //                     auto interpolated_normal = interpolate(alpha, beta, gamma, t.normal[0], t.normal[1], t.normal[2], 1);
    //                     auto interpolated_texcoords = interpolate(alpha, beta, gamma, t.tex_coords[0], t.tex_coords[1], t.tex_coords[2], 1);
    //                     auto interpolated_shadingcoords = interpolate(alpha, beta, gamma, view_pos[0], view_pos[1], view_pos[2], 1);
    //                     fragment_shader_payload payload(interpolated_color, interpolated_normal.normalized(), interpolated_texcoords, texture ? &*texture : nullptr);
    //                     payload.view_pos = interpolated_shadingcoords;
    //                     COLOR[count] = fragment_shader(payload);
    //                 }
    //             }
    //         }
    //         if (insideFlag == true && depthFlag == true)
    //         {
    //             Vector3f pixel_color = (COLOR[0] + COLOR[1] + COLOR[2] + COLOR[3]) / 4;
    //             set_pixel(Vector2i(x, y), pixel_color);
    //         }
    //     }
    // }
